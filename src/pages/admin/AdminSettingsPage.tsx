...
// Generate hours for port operation times (0-23)
const operationHours = Array.from({ length: 24 }, (_, i) => ({
  value: String(i),
  ...
  return (
    <LayoutShell showSidebar={true} role="ADMIN">
      <div className="space-y-6 max-w-2xl">
        ...
        <div className="glass-section p-6 space-y-5 section-animate">
          <h2 className="text-lg font-semibold text-foreground">Port Operation Hours</h2>
          
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label htmlFor="portStartHour">Start Hour</Label>
              <Select
                value={String(settings.portStartHour)}
                onValueChange={(value) => setSettings({ ...settings, portStartHour: Number(value) })}
              >
                <SelectTrigger className="glass-control">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="glass-strong border-[rgba(87,106,255,0.25)] max-h-60">
            ...
            <div className="space-y-2">
              <Label htmlFor="portEndHour">End Hour</Label>
              <Select
                value={String(settings.portEndHour)}
                onValueChange={(value) => setSettings({ ...settings, portEndHour: Number(value) })}
              >
                <SelectTrigger className="glass-control">
                  <SelectValue />
                </SelectTrigger>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              min={1}
              max={20}
              value={settings.numberOfTerminals}
              onChange={(e) => setSettings({ ...settings, numberOfTerminals: Number(e.target.value) })}
              className="glass-control"
            />
            <p className="text-sm text-muted-foreground">Number of terminals operating at the port</p>
          </div>
        </div>
        ...
        <div className="glass-section p-6 space-y-5 section-animate">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">Duration of each appointment slot</p>
            </div>

            <div className="space-y-2">
              <Label htmlFor="pauseDuration">Pause Between Slots</Label>
              <Select
                value={String(settings.pauseDurationMinutes)}
                onValueChange={(value) => setSettings({ ...settings, pauseDurationMinutes: Number(value) })}
              >
                <SelectTrigger className="glass-control">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="glass-strong border-[rgba(87,106,255,0.25)]">
                  {pauseDurations.map((dur) => (
                    <SelectItem key={dur.value} value={dur.value}>
                      {dur.label}
                    </SelectItem>
                  ))}
              ...
              <p className="text-sm text-muted-foreground">Break time between slots</p>
            </div>
          </div>

          <div className="space-y-2">
            <Label htmlFor="defaultCapacity">Default Capacity per Slot</Label>
            <Input
              id="defaultCapacity"
              type="number"
              min={1}
              ...
              className="glass-control"
            />
            <p className="text-sm text-muted-foreground">Maximum number of bookings per time slot</p>
          </div>
        </div>
